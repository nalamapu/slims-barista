...
<nav class="nav nav-pills mt-3 mx-2">
    <?php foreach( $nav as $label => $prop): ?>
            <?php 
            $active = ($label === 'Daftar Terbaru' && !isset($_GET['section'])) ? 'active font-weight-bold' : NULL;
            ...
            ?>
            <a onclick="navClick(this)" class="nav-link notAJAX <?= $active ?>" data-target="<?= $prop[2] ?>" href="#">        
                <?= $prop[1] .' '. $label ?>
            </a>
    <?php endforeach; ?>
</nav>